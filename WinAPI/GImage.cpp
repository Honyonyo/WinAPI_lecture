...
{
	HDC hdc = GetDC(_hWnd);
	_blendFunc.BlendFlags = 0;				//빠르게 올릴거니까 블렌드옵션 따로 없음
	_blendFunc.AlphaFormat = 0;				//특정조건에서발동이므로 따로 사용 안함
	_blendFunc.BlendOp = AC_SRC_OVER;		//복사될 개체에 먼저 알파값 연산을 해주고 혼합연산으로 비트맵에 알파값(더 정확히는 픽셀값)을 넣겠다. - 혼합값 넣어주는 옵션
...
//이미지 클리핑 
// ㄴ원본 이미지 좌표로부터 가로, 세로 크기를 잘라 내가 원하는 좌표에 출력
...
void GImage::alphaRender(HDC hdc, BYTE alpha)
{
	...
	//적용하려는 대상 객체 = 인자로 넘겨줄 alpha값 넘겨주겠다.
	_blendFunc.SourceConstantAlpha = alpha;

	if (_isTrans) {
		//1. 출력해야할 이미지를 블랜드 이미지에 복사한다.
		BitBlt
		(
			_blendImage->hMemDc,
			0,0,
...
//출력될 x, y, 복사시작x, y, 출력너비width, height, 불투명도
...
{
	_imageInfo->currentFrameX = (currentFrameX > _imageInfo->currentFrameX)
								? _imageInfo->maxFrameX : currentFrameX;
	_imageInfo->currentFrameY = (currentFrameY > _imageInfo->currentFrameY)
...
void GImage::loopRender(HDC hdc, const LPRECT drawArea, int offsetX, int offsetY)
{
	...
	for (int y = 0; y < drawAreaH; y += sourHeight) {
		...
		//소스 영역이 그리는 화면을 넘어갔다면 (화면밖으로 나갔다면)
